 ...
 *   ANTHROPIC_API_KEY   Optional for LLM naming (skipped if not provided)
 *
 * Options:
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Usage: TEST_VAULT_PATH=~/Documents/MyVault OPENAI_API_KEY=xxx npx tsx scripts/run-full-pipeline.ts [options]
...
`);
  ...
  if (!anthropicApiKey) {
    console.error(`LLM: Disabled (no ANTHROPIC_API_KEY)`);
  }
  console.error('');
  ...
  const llmProvider = anthropicApiKey
    ? new AnthropicLLMAdapter(anthropicApiKey, {
  ...
  if (persistedResult) {
    ...
    for (const cluster of sortedClusters.slice(0, 10)) {
      const name = cluster.canonicalName ?? cluster.candidateNames[0] ?? cluster.id;
      const quizzable =
        cluster.quizzabilityScore !== undefined && cluster.quizzabilityScore >= 0.4 ? 'Q' : ' ';
      const scoreStr =
        cluster.quizzabilityScore !== undefined ? ` score: ${cluster.quizzabilityScore.toFixed(2)}` : '';
      console.error(`  [${quizzable}] ${name} (${cluster.noteIds.length} notes${scoreStr})`);
    }
  }
}
