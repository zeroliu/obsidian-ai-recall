	...
	for (let i = 0; i < numClusters; i++) {
		...
		result.push(
			createCluster({
				...
				candidateNames: [...cluster.candidateNames, `Part ${i + 1}`],
 ...
 */
function sortNotesByConnectivity(noteIds: string[], resolvedLinks: ResolvedLinks): string[] {
 ...
 */
export function mergeSmallClusters(clusters: Cluster[], config: ClusteringConfig): Cluster[] {
	const small: Cluster[] = [];
	const result: Cluster[] = [];
 ...
 */
function mergeSmallClustersGreedy(clusters: Cluster[], config: ClusteringConfig): Cluster[] {
